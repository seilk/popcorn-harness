...
Start
  |
  +-- Count capabilities mapped from Step 3, item 2 (the "Map" step)
  |
  ...
  |
  +-- count == 2 or 3 AND no significant ambiguity?
  |     +-- YES -> Tier 2 (Standard Pop)
  |
  +-- count == 4 or 5?
  |     +-- YES -> Tier 3 (Full Pop)
  |
  +-- Ambiguity override: any of these -> Tier 3 (regardless of count)
...
| Condition | Action |
|-----------|--------|
| Capability output reveals 1 new required capability | Announce addition ("Adding: [capability]"), then load. Never silent. |
...
-> Before assigning a tier, ask a clarifying question first
...
-> After the user answers, proceed with normal tier assignment from Step 2
